# Inserción Laboral – Técnico Microinformático de Campo

Este repositorio contiene mi proyecto de Inserción Laboral para Grado Medio en Sistemas Microinformáticos y Redes (SMR).

## 👨‍💻 Sobre mí
Soy estudiante de 1º de SMR y estoy formándome para trabajar en el sector de la informática. Me interesa especialmente el soporte técnico, el mantenimiento de equipos y las redes. Me considero una persona responsable, con ganas de aprender y mejorar cada día.

---

## 🛠️ Habilidades Técnicas (Hard Skills)

- Montaje y mantenimiento de equipos informáticos  
- Instalación y configuración de sistemas operativos Windows  
- Redes básicas (IP, router, cableado, LAN)  
- Resolución de incidencias de hardware y software  
- Uso de herramientas ofimáticas  
- Configuración básica de aplicaciones y periféricos  

---

## 🤝 Habilidades Personales (Soft Skills)

- Trabajo en equipo  
- Comunicación  
- Organización  
- Responsabilidad  
- Aprendizaje rápido  
- Atención al usuario  

---

## 🌍 Idiomas

- **Español:** Nativo  
- **Catalán:** Nativo  
- **Inglés:** Intermedio  

---

## 🧪 Experiencia (Académica / Práctica)

Aunque todavía no tengo experiencia laboral, sí cuento con experiencia práctica adquirida durante mis estudios:

- Montaje y desmontaje de ordenadores en clase  
- Instalación de sistemas operativos y software  
- Configuración de redes locales  
- Resolución de problemas básicos en equipos  
- Mantenimiento de ordenadores de familiares y amigos  
- Configuración de routers y redes domésticas  

---

## 📂 Contenido del repositorio

- Presentación en Canva  
- Tarjeta de presentación  
- Información sobre mis habilidades  
- Experiencia académica  
- Enlace a mis proyectos (si añado más en el futuro)

---

## 🎯 Objetivo Profesional

Seguir formándome y desarrollarme como técnico informático, especialmente en soporte técnico, mantenimiento de equipos y redes. Mi objetivo es aplicar mis conocimientos en un entorno real y crecer dentro del sector TIC.
